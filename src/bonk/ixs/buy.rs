...
const BUY_EXACT_IN_DISCRIMINATOR: [u8; 8] = [0xfa, 0xea, 0x0d, 0x7b, 0xd5, 0x9c, 0x13, 0xec];
...
pub fn build_buy_exact_in_ix(
    ...
    data.extend_from_slice(&BUY_EXACT_IN_DISCRIMINATOR); // 8字节
    data.extend_from_slice(&amount_in.to_le_bytes()); // u64
    data.extend_from_slice(&minimum_amount_out.to_le_bytes()); // u64
