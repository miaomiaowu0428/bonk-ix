...
// ]
//
pub fn build_sell_exact_out_ix(
    payer: Pubkey,
    ...
    // BUY_EXACT_IN_DISCRIMINATOR 需用idl里对应的8字节
    let mut data = vec![];

    data.extend_from_slice(&SELL_EXACT_OUT_DISCRIMINATOR); // 8字节
...
pub fn build_sell_exact_in_ix(
    ...
    data.extend_from_slice(&SELL_EXACT_IN_DISCRIMINATOR); // 8字节
    data.extend_from_slice(&amount_in.to_le_bytes()); // u64
    data.extend_from_slice(&min_amount_out.to_le_bytes()); // u64
    data.extend_from_slice(&share_fee_rate.to_le_bytes()); // u64
